...
fn get_cross_call() {
    ...
    storage_contract_client.put(&u1, &bytes![&env, 0x48656c6c6f20536f726f62616e21]);

    // We invoke the `inv_get()` function using our `CrossContractCall` client,
    // supplying the `DataStore` contract address and our test user as
    // arguments. We expect it to return the same data we already stored in the
    // `DataStore` contract.
    assert_eq!(
