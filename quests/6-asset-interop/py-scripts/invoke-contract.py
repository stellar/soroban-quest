...
1. You need to follow [this tutorial](https://soroban.stellar.org/docs/getting-started/deploy-to-a-local-network)
...
if get_transaction_status_data.status == TransactionStatus.SUCCESS:
    result = stellar_xdr.SCVal.from_xdr(
        get_transaction_status_data.results[0].xdr)  # type: ignore
    output = [x.sym.sc_symbol.decode()
              for x in result.obj.vec.sc_vec]  # type: ignore
